...
Но мы можем создать цифровой документ в ipfs, который связывает цифровой профиль пользователя и его достижения в nft.
Этот документ будет зашифрован публичным ключом самого пользователя и доступен только для него.
Соответственно документ без его разрешения не может просматриваться другим пользователем.
При этом пользователь может поделиться любой необходимой открытой частью из документа .
